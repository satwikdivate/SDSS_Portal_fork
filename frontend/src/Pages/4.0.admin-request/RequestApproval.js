  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        const usersDetailsPromises = allRequests.map((request) =>
          getById(request.user)
        );
  ...
  const renderRequestSection = (requests) => {
    ...
    return requests.map((request) => (
      <div key={request._id} className="request-item">
        <p>
          User Name:{" "}
          {userDetailsMap[request.user]?.firstName +
            " " +
            userDetailsMap[request.user]?.lastName}
        </p>
        ...
        <div className="button-container">
          {request.status === "Pending" && (
            <>
              <button
                id="accept"
                onClick={() => handleAccept(request.user, request._id)}
              >
                Accept
              </button>
              <button
                id="reject"
                onClick={() => handleReject(request.user, request._id)}
              >
                Reject
              </button>
            </>
